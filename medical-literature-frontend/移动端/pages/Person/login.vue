	...
	<view class="container" :style="(ispdFocus || isuserFocus)?containerHeight.focus:containerHeight.blur">
		<view class="input-area">
			...
			<view class="input-text" :style="isuserFocus?input_boder_style.focus:input_boder_style.blur">
				...
				<u-input :focus="un" v-model="userLoginInfo.userName" type="text" :height="100" placeholder="" :custom-style="loginStyle"
			...
			<view class="input-text" :style="ispdFocus?input_boder_style.focus:input_boder_style.blur">
				...
				<u-input :focus="pd" v-model="userLoginInfo.password" type="password" :password-icon="false" :height="100"
				 placeholder="" :maxlength="pdMaxLength" :custom-style="loginStyle" :clearable="false" @focus="pdFocus" @blur="pdBlur" />
			...
			<view class="btn">
				<u-button size="default"  @click="goFindPass" :custom-style="login_btn_style.able">找回密码</u-button>
			</view>
			<view class="btn">
				<u-button size="default"  @click="goRegister" :custom-style="login_btn_style.able">注册</u-button>
			</view>
...
<script>
	import {ReqLogin} from '@/utils/request/index.js'
	export default {
		name:'login',
		data() {
			return {
		...
		methods: {
			...
			goFindPass(){
				this.$emit('goFindPass')
			},
			async login() {
				// 密码正确
				this.isLogining = true
				
				let res = await ReqLogin(this.userLoginInfo.userName,this.userLoginInfo.password)
				if(res.code == 0 ){
					...
					uni.setStorageSync('userName',this.userLoginInfo.userName)
					this.$emit('loginHandle',this.userLoginInfo.userName)
				}
			}
		},
		watch: {
